...
La actividad está preparada para compilarse y ejecutarse dentro de un contenedor Docker.
...
```bash
docker run --rm -v "$(pwd):/output" disk-scheduler 2500
```

| Parte del comando | Qué hace |
|---|---|
| `--rm` | Elimina el contenedor al terminar |
| `-v "$(pwd):/output"` | Monta la carpeta actual dentro del contenedor para que el `data.js` generado quede disponible en tu máquina |
| `disk-scheduler` | Nombre de la imagen |
| `2500` | Posición inicial del cabezal (puede cambiarse) |
...
Una vez generado el `data.js`, abrir `index.html` en el navegador para ver las gráficas.
...
El ejecutable recibe la posición inicial del cabezal como argumento al correr el contenedor.
